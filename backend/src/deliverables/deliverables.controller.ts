...
export class DeliverablesController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Deliverable completed successfully',
  })
  ...
  async complete(@CurrentUser() user: UserPayload, @Param('id') id: string) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Deliverable reopened successfully',
  })
  @ApiResponse({ status: 404, description: 'Deliverable not found' })
  @ApiResponse({ status: 403, description: 'Cannot reopen this deliverable' })
  async reopen(@CurrentUser() user: UserPayload, @Param('id') id: string) {
